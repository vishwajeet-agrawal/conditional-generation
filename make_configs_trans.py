...
if __name__ == '__main__':
    ...
    for j, r in data_configs.iterrows():
        ...
        for i, row in tqdm(configs.iterrows()):
            ...
            config_.train.eval = om.create({})
            config_.train.eval.consistency = om.create({})
            config_.model.aggregator = om.create({'type': 'transformer'})
            config_.model.n_heads = int(row['n_heads'])
            config_.model.n_layers = n_layers
            config_.model.embedding_dim = int(row['embedding_dim'])

            config_.model.eval = config_.train.eval
            row_ = pd.DataFrame({'datapath': config_.data.path,
                                    ...
                                    'n_heads' : config_.model.n_heads}, index=[0])
            ...
            df = pd.DataFrame(configs_all)
            df = df.drop_duplicates()
            df.to_csv(args.configs_save, index = False)
